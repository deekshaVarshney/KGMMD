...
class BaseRNNDecoder(nn.Module):
	...
	def _input_size(self):
		...
		# Use kb
		if self.use_kb:
			_in_size = _in_size + self.kb_size
		# Use aspect
		if self.use_aspect:
			_in_size = _in_size + self.aspect_size
		# Use state vector
...
class DecoderRNN(BaseRNNDecoder):
	def __init__(self, vocab_size, embedding_size,
				 ...
				 kb_size=None, use_aspect=False, aspect_size=None,celeb_vec_size=None, state_size=None):
	...
	def forward_step(self, x, h, 
		...
		if self.use_kb:
			...
			combined_kb_vec = kb_vec
			decoder_input = torch.cat([decoder_input, combined_kb_vec], 2)
		if self.use_aspect:
			# state_vec: [batch_size, state_size] => [batch_size, 1, state_size]
			aspect_vec = aspect_vec.unsqueeze(1)
			# print(aspect_vec.size())
			# celeb_vec = celeb_vec.unsqueeze(1)
			combined_aspect_vec = aspect_vec
			decoder_input = torch.cat([decoder_input, combined_aspect_vec], 2)
	...
	def forward(self, inputs=None, init_h=None, encoder_outputs=None, 
				...
				image_outputs = None, kb_vec = None, aspect_vec = None, celeb_vec = None, 
		...
		if not decode:
			...
			for i in range(seq_len):
				...
				out, h, input_feed = self.forward_step(x, h, encoder_outputs, 
										...
										kb_vec, aspect_vec,celeb_vec, state_vec)
				out_list.append(out)
