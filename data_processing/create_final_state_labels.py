...
def create_state_data(out_dir_path, data_type):
	...
	index_map = pkl.load(open(state_map_file,'rb')) # Index map
	state_labels = []
	with open(state_type_file, 'rb') as state_file:

		for state_line in state_file:
			# print(state_line)
			state = state_line.decode('utf8').split(',')[2] # question type
			state_ = state.split('_')
			if state_[0]=='like' or state_[0]=='do':
				state = 'like'			
			# state = ','.join(state_line.split(',')[:-1])
			state_label = int(index_map[state])
	...
	with open(state_label_pkl, 'wb') as pkl_file:
