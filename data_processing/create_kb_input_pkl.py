...
def read_vocab(data_dir):
	...
	word_counter = pkl.load(open(kb_vocab_stats_file,'rb'))
	...
	for (word, count) in vocab_count:
		if not word in vocab_dict:
			vocab_dict[word] = i
			i += 1
	inverted_vocab_dict = {word_id:word for word, word_id in vocab_dict.items()}
...
def convert_kb(data_dir, data_type, vocab_dict, pad_id=0, unk_id=2):
	kb_text_file_path = os.path.join(data_dir, data_type+"_kb.txt")
	aspect_text_file_path = os.path.join(data_dir, data_type+"_aspect.txt")
	...
	aspect_out_file_path = os.path.join(data_dir, data_type+"_aspect_text_both.pkl")
	# aspect_vocab_stats_file = os.path.join(data_dir, "vocab_stats.pkl")
	aspect_vocab_file = os.path.join(data_dir, "vocab.pkl")
	aspect_dic = pkl.load(open(aspect_vocab_file,'rb'))

	wpt = nltk.WordPunctTokenizer()

	kb_text_file = open(kb_text_file_path,'r')
	aspect_text_file = open(aspect_text_file_path,'r')

	# kb_lines = kb_text_file.readlines()
	# print('dd',len(kb_lines))
	...
	aspect_utterance = []
	aspect_seq_len = []
	for utterance in aspect_text_file:
		utterance = utterance.strip()
		utterance_words = wpt.tokenize(utterance)
		# print(utterance_words)
		utterance_word_ids = []
		for word in utterance_words:
			if word in aspect_dic:
				word_id = aspect_dic[word]
			elif word=='':
				word_id =pad_id #Corner case
			else:
				word_id =unk_id
			utterance_word_ids.append(word_id)
		length_utterance, utterance_word_ids = pad_or_clip_utterance(utterance_word_ids, max_len=10)
		aspect_utterance.append(utterance_word_ids)
		aspect_seq_len.append(length_utterance)
	# length_utterance, utterance_word_ids = pad_or_clip_utterance(utterance_word_ids)
	aspect_corpora = [aspect_seq_len, aspect_utterance]


	print('kk',len(kb_corpora[1]))
	print('aspect',len(aspect_corpora[1]))


	with open(kb_out_file_path, 'wb') as f:
		pkl.dump(kb_corpora, f, protocol=pkl.HIGHEST_PROTOCOL)	

	with open(aspect_out_file_path, 'wb') as f:
		pkl.dump(aspect_corpora, f, protocol=pkl.HIGHEST_PROTOCOL)	
