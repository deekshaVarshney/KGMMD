...
class DataBuilder():	
	def __init__(self, context_size, max_images, unk_id, pad_id, start_id, end_id, image_rep_size, 
	...
	def write_model_data(self, out_dir_path, vocab_file, data_type):
		...
		self.annoy_file = os.path.join(out_dir_path, self.data_type+"_annoy.pkl")
		...
		self.dialogue_pkl_file_tf = os.path.join(out_dir_path, self.data_type+"_tf.pkl")
	...
	def convert_for_model(self, context_text_file, context_image_file, 
		...
		with open(context_text_file) as textlines, open(context_image_file) as imagelines,\
			...
			# for text_context, target in zip(textlines, targetlines):
				binarized_text_context_seq = []
				binarized_text_context_len = []
				binarized_image_context_seq = []
				binarized_image_context_len = []
				...
				context_texts, unknownss, context_lengths = self.text_processing_tf(
																	utterances, unknowns)
				# Image contexts
				...
				for image_turn in image_turns:
					...
					# print(context_image, index)
					# self.annoy_pkl(context_image, index, dic)
					for url in context_image:
						if url != "" and url != "null":
							dic[url] = index
							index += 1
							# print(url, index)
				...
				target_seq, unknowns, target_length = self.text_processing(
																target, unknowns)
				target_seqs, unknownss, target_lengths = self.text_processing(
																target, unknowns)
				...
				binarized_corpus.append([text_context_seq, 
										...
										target_seq, 
										target_length])
				binarized_corpus_tf.append([[context_texts], 
										[context_lengths], 
										image_context_seq, 
										image_context_len, 
										target_seqs, 
		...
		self.save_to_pickle(binarized_corpus_tf, self.dialogue_pkl_file_tf)
		# print(len(dic.keys()))
		self.save_to_pickle(dic, self.annoy_file)
	...
	def text_processing_tf(self, utterance, unknowns):
		utterance = utterance.strip()
		utterance_words = self.wpt.tokenize(utterance)
		utterance_word_ids = []
		for word in utterance_words:
			if word in self.vocab_dict:
				...
				unknowns += 1
			utterance_word_ids.append(word_id)
		length_utterance, utterance_word_ids = self.pad_or_clip_utterance_tf(utterance_word_ids)
		# utterance_word_ids is a list of max_len
		return utterance_word_ids, unknowns, length_utterance

	def pad_or_clip_utterance_tf(self, utterance):
		# utterance: list of (<start> sent <end>)
		length_utterance = len(utterance)
		if length_utterance>=(self.max_len):
			# end_token = utterance[-1]
			utterance = [self.start_id] + utterance[:(self.max_len-1)]
			utterance.append(self.end_id)
			seq_length = self.max_len
		else:
			pad_length = self.max_len - length_utterance -1 # pad = max - (length+end)
			utterance = [self.start_id] + utterance + [self.end_id] +[self.pad_id]*pad_length
			seq_length = length_utterance + 1
		return seq_length, utterance
	...
	# def annoy_pkl(self, image_filename,index,dic):
...
if __name__=="__main__":
	...
	data_builder = DataBuilder(args.context_size, args.max_images, args.unk_id, 
								args.pad_id, args.start_id,args.end_id, args.image_rep_size, 
