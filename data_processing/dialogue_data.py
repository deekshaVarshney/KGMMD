...
class PrepareData():
	...
	def prepare_data(self, dir_path, out_dir_path, vocab_file=None, vocab_stats_file=None, 
		...
		self.aspect_file = os.path.join(out_dir_path, self.data_type+"_aspect.txt")
		...
		print('sssss',self.read_dir_path)
	...
	def read_jsonfile(self, json_file, create_vocab, write_all):
		...
		for utterance in dialogue:
			...
			if 'kb' in utterance['utterance']:
				...
				global_kb_line = kb_line['kb']  # Repeats KB line for next conversations
				if isinstance(global_kb_line, list):
					
					global_kb_line = "null"

				# if global_kb_line_list != 'null':
					# for g in global_kb_line_list:
					# 	for d in g:
					# 		if g != '':
					# 			print(g)

					# global_kb_line = (' ').join([d['review_body'].strip().replace('\n','') for g in global_kb_line_list for d in g if d != ''])
					# global_kb_line

			if 'aspect' in utterance['utterance']:
				aspect_line = utterance['utterance']
				global_aspect_line = aspect_line['aspect']
				# print(global_aspect_line)
				if global_aspect_line != 'null':
					global_aspect_line = (' ').join(global_aspect_line)
			...
			if speaker=="system":
				...
				aspect_list.append(global_aspect_line)
					...
					for images in dialogue_instance:
						...
						if image_context is None:
							...
							if images != "null":
								...
								image_context = image_context+"|"+images
					# print(image_context)
		...
		self.write_list_to_file(self.aspect_file, aspect_list)
